...
def main(args):
    ...
    cost_redux = f"  -> Cost Reduction: {(baseline_metrics['cost'] - cost) / baseline_metrics['cost'] * 100:.2f}%"
    time_redux = f"  -> Time Reduction: {(baseline_metrics['time'] - time) / baseline_metrics['time'] * 100:.2f}%"
    wait_redux = f"  -> Wait Reduction: {(baseline_metrics['wait'] - wait) / baseline_metrics['wait'] * 100:.2f}%"
    apc_redux = f"  -> Agents/Case Reduction: {(baseline_metrics['agents_per_case'] - apc) / baseline_metrics['agents_per_case'] * 100:.2f}%"
    ta_redux = f"  -> Total Agents Reduction: {(baseline_metrics['total_agents'] - ta) / baseline_metrics['total_agents'] * 100:.2f}%"
